...
class Table(Deck):
    ...
    def deal_on_table(self, qty):
        for i in range(qty):
            dealed = Deck.deck_cards.pop()
            self.on_table.append(dealed)
...
class Player(Deck):
    ...
    def get_card(self, qty):
        ...
        print(f'>>> To {self.name} was dealed {qty} new cards.<<<')
...
class Winner(Player):
    ...
    def two_pair(self, player):
        """rank: 8, classification: ?"""
        pass

    def one_pair(self, player):
        """rank: 9, classification: ?"""
        pass
